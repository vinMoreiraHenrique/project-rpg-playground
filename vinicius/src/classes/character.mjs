export class Character {
  ...
  experienceForNextLevel() {
    return (
      (50 * this.level * this.level * this.level -
        150 * this.level * this.level +
        400 * this.level) /
      3
    );
  }
  getStats() {
    ...
    this.vocation.spells.forEach((spell) => {
      const {
        name,
        minLevel,
        minMagicLevel,
      ...
      console.log(
        `----> ${name.toUpperCase()} -\n Nível mínimo: ${minLevel}\n Nível mínimo de magia: ${minMagicLevel}\n Mana: ${manaCost}\n Dano: ${baseDamage}\n Tipo de dano: ${damageType}\n`
      );
    });
    console.log(
      `EXP TO NEXT LEVEL: ${this.experienceToLevelUp - this.experience}`
    );
  }
  ...
  spellCast(spell, monster) {
    let spellTrue = this.spells.map(
      (element) => element.name.includes(spell) != false && element
    );
    ...
    if (minLevel || minMagicLevel) {
      ...
      console.log(
        `Você causou ${spellCombatDamage} de dano ao monstro ${monster.name}, e agora ele está com ${monster.life} de vida.`
      );
    }
  }
  ...
  levelUp() {
    ...
    console.log(
      `Parabéns, ${this.name} você foi do nível ${oldLevel} para o nível ${this.level}!`
    );
